class Api::V1::UsersController < ApplicationController
  ...
  def create
    ...
    if @user.valid? 
      ...
      render json: { user: UserSerializer.new(@user), token: token, errorStatus: false }, status: :accepted
    else
      # if user is not valid - render error messages (rails validation messages) and status code

      render json: { header: "You need to fulfill these #{@user.errors.full_messages.count} requirements", error: @user.errors.full_messages, errorStatus: true }, status: :unauthorized 
    end
  end
